...
# Build script for Rules Engine Frontend
set -e
...
check_docker() {
    ...
    if ! docker info > /dev/null 2>&1; then
        print_status 1 "Docker is not running. Please start Docker and try again."
    fi
...
build_docker_image() {
    ...
    if docker build -t "$full_image_name" .; then
        print_status 0 "Docker image built: $full_image_name"
    else
...
push_docker_image() {
    ...
    if docker push "$full_image_name"; then
        print_status 0 "Docker image pushed: $full_image_name"
    else
        print_status 1 "Failed to push Docker image"
    fi
}
...
# Main execution
main() {
    echo -e "${BLUE}🚀 Building Rules Engine Frontend${NC}"
    echo "=================================="
    ...
    echo "2. Or run locally: docker run -p 3000:3000 ${REGISTRY}/${IMAGE_NAME}:${IMAGE_TAG}"
}
